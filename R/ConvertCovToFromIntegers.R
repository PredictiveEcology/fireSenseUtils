...
#'
#' @param annualList Named list of `data.table`s of covariates that vary annually. Must
#'   have a `pixelID` column. List name is year.
#' @param nonAnnualList Named list of `data.table`s of covariates that do not vary each year. Must
#'   have a `pixelID` column. List name is a concatenated set of years separated by underscore.
#' @param fireBufferedList Named list of `data.table`s of buffered perimeters with pixelID. Must
#'   have a `pixelID` column. List name is `yearXXXX` where `XXXX` is the 4 digit year. Column
#'   names must be `pixelID`, `buffer` with every row a `1` or `0` as to whether that pixel
#'   is in or outside the buffer (`1` is in the buffer!), and `ids` with a unique indentifier
#'   for each fire.
...
#' @param param Order Character vector of the order of covariates that will be used.

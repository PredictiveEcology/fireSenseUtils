#' Cleaning up the polygon centroids
#' 
...
#' @export
#' @param cent List of polygon centroids as \code{SpatialPointsDataFrame}
...
#'   in the \code{cent} 
#' @param ras The raster that created the pixelIDs in the \code{buff}
#' @importFrom sp spTransform SpatialPointsDataFrame
#' @importFrom raster cellFromXY crs
#' @importFrom data.table data.table setkeyv
#' @importFrom SpaDES.tools spread
cleanUpPolyCentroids <- function(cent, buff, ras, idCol = "NFIREID") {
  purrr::pmap(list(cent = cent,
              ...
              .f = function(cent, buff) {
                ...
                centDT <- data.table(pixelID = cellFromXY(spTransform(polyCentroids, crs(ras)), object = ras),
                ...
                if (NROW(notInAFire)) {
                  ...
                  replacementCentroids <- dfep[,  
                       ...
                       by = "id"]
                  # replacementCentroids <- dfep[, list(centroidIndex = .I[which.min(dists)]), by = "id"]

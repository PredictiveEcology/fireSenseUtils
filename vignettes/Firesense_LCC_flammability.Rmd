...
## The data servers (ftp.maps.canada.ca for LCC, cwfis.cfs.nrcan.gc.ca for the
## fire polygons) are routinely unreachable from R-CMD-check runners, which made
## the live build die with "Failed to connect to ... port 443". Probe both hosts
## with a short HEAD request up front and skip the live chunks when either is
## down — the code stays visible as documentation, same as the macOS path. Any
## probe error (including curl being unavailable) degrades to skip, never error.
lccUrl <- paste0(
...
.canReach <- function(url, timeoutSec = 20) {
  tryCatch(
    {
      h <- curl::new_handle(nobody = TRUE, connecttimeout = timeoutSec)
...
.reachable <- requireNamespace("curl", quietly = TRUE) &&
  .canReach(lccUrl) && .canReach(fireUrl)

knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  echo = TRUE,
  eval = .canExtract && .reachable
)
```
...
## lccUrl is defined in the setup chunk (used there for the reachability probe)
lcc <- prepInputs(
...
## fireUrl is defined in the setup chunk (used there for the reachability probe)

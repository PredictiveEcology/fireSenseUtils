...
library(knitr)
## This vignette ships pre-cropped fixtures (inst/extdata) rather than
## downloading its inputs, so it needs no network and no `archive` extraction.
## The live-download code it replaces is preserved verbatim below, commented,
## so the real workflow stays visible as documentation.
##
## Why fixtures: the live version pulled ~2.7 GB per run -- 778 MB of fire
## polygons plus a 1.9 GB land-cover raster -- to analyse a 50 km square. Under
## a full CI matrix several legs downloaded that concurrently from two NRCan
## servers, and truncated responses produced corrupt geometry that surfaced as
## `ParseException: Unexpected EOF parsing WKB` inside sf::st_crop(). A HEAD
## reachability probe cannot catch that: the servers are up, the transfer is
## simply cut short.
...
library(data.table)
library(ggplot2)
## reproducible is not needed by the fixture path; the commented live block
## below uses reproducible::prepInputs(), so load it if you run that.
library(sf)
library(terra)
```
...
## ---------------------------------------------------------------------------
## Inputs: pre-cropped fixtures shipped with the package (a 50 km square in
## central Saskatchewan). To run this against the live national datasets
## instead, use the commented block below.
## ---------------------------------------------------------------------------
lcc <- terra::rast(
  system.file("extdata", "lcc_studyArea.tif", package = "fireSenseUtils")
)
firePolygons <- sf::read_sf(
  system.file("extdata", "firePolygons_studyArea.gpkg", package = "fireSenseUtils")
)

## --- LIVE VERSION (not run) -------------------------------------------------
## Land Cover of Canada 2015 (1.9 GB zip) and the National Fire Database
## polygons (778 MB zip). Both are national-extent; `cropTo`/`st_crop` reduce
## them to the study area. Remove the crop to analyse the full country.
##
## lccUrl <- paste0(
...
## )
## fireUrl <- paste0(
##   "https://cwfis.cfs.nrcan.gc.ca/downloads/nfdb/fire_poly/",
...
##
## ## study area: 50 km square in central Saskatchewan
## studyArea <- vect(
...
##
## NOTE: NFDB_poly.zip contains TWO layers -- 1972to2020 and 2021to2024.
## `sf::read_sf` reads only the first, so the live version above silently
## excludes fires after 2020. The fixture combines both, then filters
## YEAR > 2004, and therefore covers 2007-2024.
##
## GDAL can also read these archives without downloading them, via
## `/vsizip//vsicurl/<url>`, which fetches only the byte ranges needed. That
## is how the fixtures here were built.
## ---------------------------------------------------------------------------
